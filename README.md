...
`prepare-data` 用于把当前本地库快速补到可研究状态：按指定股票池刷新公开行情，可选同步公开财报和行业标签，随后重新评分并输出 `outputs/candidates_ready.csv` 与 `outputs/daily_report_ready.md`，同时在 `daily-runs` 中保存本次准备数据的摘要。公开财报或行业个别代码失败会留存样例但不中断；若公开行情没有成功导入则整次准备会标记失败。
