...
`data-health` 同时汇总实时行情时效（带价格行情总数、近一工作日覆盖、可能过期数量和最近价格日期）、按公告日边界统计的财务披露覆盖和当前评分覆盖的公开行业标签，并在 CLI 与 Web 顶部给出“数据准备度”和下一步补数建议；行业统计只用于当前研究展示，不进入历史回测。

`prepare-data` 用于把当前本地库快速补到可研究状态：按指定股票池刷新公开行情，可选同步公开财报、行业标签和 `--public-announcements` 公告，随后重新评分并输出 `outputs/candidates_ready.csv` 与 `outputs/daily_report_ready.md`，同时在 `daily-runs` 中保存本次准备数据的摘要。财报、行业或公告的个别代码失败会留存样例但不中断；若公开行情没有成功导入则整次准备会标记失败。
...
本地观察记录保存在项目 SQLite 的 `stock_notes` 表里，不会写回同花顺文件。可以在个股详情页编辑，也可以进入 `http://127.0.0.1:8765/notes` 查看“观察、持有、回避、复盘”分组后的观察池。观察池支持搜索代码/名称/标签/备注、按更新时间/评分/涨跌幅/现价/代码排序、查看单只报价时间、导出 CSV，并可在列表页删除本地记录。命令行同样支持写入、筛选、搜索、排序和删除：
