...
- 观察池页面、观察池 CSV、`notes` CLI 和每日 Markdown 报告均展示单只报价时间；日报明确将其解释为报价观测时间，而不是报告生成时间
- 每日 Markdown 报告头部还汇总日线与实时行情整体时效，先提示整份研究数据是否滞后，再保留候选逐只报价时间供复核
- `prepare-data` 可选同步公开公告，并把公告状态/保存数与其他数据补数一起写入每日记录；公告网络失败不阻断行情、评分和候选报告
- AI 观点变化对比
